...
def parse_args():
    ...
    parser.add_argument('--mesh_bs', type=int, default=720, help='Batch size for mesh preparation')
...
def load_inputs(args, MAX_IMG_W=1920, MAX_IMG_H=1080):
    ...
    if inputs_type == 'video':
        ...
        #inputs_meta['seq_name'] = inputs_path.stem
        # --- Custom load only first 128 frames ---
        #cap = cv2.VideoCapture(str(inputs_path))
        #raw_imgs = []
        #max_frames = 128
        #frame_count = 0
        ...
        #    h, w = frame.shape[:2]
        #    if h > MAX_IMG_H:
        #        frame = flex_resize_img(frame, (MAX_IMG_H, -1), kp_mod=4)
        #    if frame.shape[1] > MAX_IMG_W:
        #        frame = flex_resize_img(frame, (-1, MAX_IMG_W), kp_mod=4)
        #    raw_imgs.append(frame)
        #    frame_count += 1
        #cap.release()
        get_logger(brief=True).info(f"📉 Trimmed video to first {len(raw_imgs)} frames.")
...
def prepare_mesh(pipeline, pd_params, B=64) -> Tuple[Dict, Dict]:
    v_skin_all, v_skel_all = [], []
    for bw in asb(total=len(pd_params['poses']), bs_scope=B, enable_tqdm=True):
        with torch.no_grad():
            skel_outputs = pipeline.skel_model(
