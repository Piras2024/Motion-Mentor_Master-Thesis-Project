...
# Guo-feature normalization stats live alongside this package (motionllm/meta/),
# resolved relative to this file so the loader is independent of the working dir.
_META_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "meta")
...
class MotionLLM(nn.Module):
    def __init__(self, args):
        ...
        self.mean = np.load(os.path.join(_META_DIR, 'mean.npy'))
        self.std = np.load(os.path.join(_META_DIR, 'std.npy'))
        ...
        self.args.vq_path = getattr(self.args, "vq_path",
                                    os.path.join(os.path.dirname(os.path.abspath(__file__)),
                                                 "..", "..", "motion_encoder",
                                                 "checkpoints", "vqvae_indomain_best.pth"))
    ...
    def forward(self, caption, motion, instruction=None, max_tgt_len=200):
    ...
    def caption(self, motion, instruction=None, temperature=1.0, top_p=1.0, do_sample=False):
        self.llm.set_adapter('m2t')
        self.llm.eval()
        ...
        from models.training_utils import DEFAULT_M2T_INSTRUCTION
        prompt = "Below is an instruction that describes a task, paired with an input that provides further context. Write a response that appropriately completes the request.\n\n"
        if instruction is None:
            instruction = DEFAULT_M2T_INSTRUCTION
        ...
        gen_kwargs = dict(max_length=200)
        if do_sample:
            gen_kwargs.update(do_sample=True, temperature=temperature, top_p=top_p)
        else:
            gen_kwargs["num_beams"] = 2
        pred = self.llm.generate(input_ids, **gen_kwargs)
    ...
    def save_model(self, path):
        ...
        for name, param in self.llm.named_parameters():
            if 'lora' in name:
                save_dict[name] = param.detach().cpu().clone()

        # save the additional token embeddings.
        # NOTE: slicing returns a view that shares storage with the full
        # embedding matrix; .clone() detaches it so torch.save serializes only
        # the ~512 added rows instead of the entire base vocabulary (~2.3 GB).
        embeddings = self.llm.get_input_embeddings().weight[self.nb_text_tokens:]
        save_dict['embeddings'] = embeddings.detach().cpu().clone()

        # save the lm_head of the additional tokens
        lm_head = self.llm.lm_head.weight[self.nb_text_tokens:]
        save_dict['lm_head'] = lm_head.detach().cpu().clone()
