...
DEFAULT_M2T_INSTRUCTION = "### Instruction:\nGenerate a caption matching the following input human motion token sequence.\n\n"

def build_one_instance_m2t(tokenizer, captions, motion, instruction=None):
    input_ids, target_ids = [], []
    bos = tokenizer.bos_token_id
    input_ids.append(bos)
    target_ids.append(-100)  # do not perform loss regression on human prompt
...
def process_batch(tokenizer, batch_of_captions, max_tgt_len, batch_of_motions, training_task, instruction=None):
    # instruction can be a single string (applied to all) or a list (one per sample)
    batch_input_ids, batch_target_ids = [], []
    for i, (caption, motion) in enumerate(zip(batch_of_captions, batch_of_motions)):
        instr = instruction[i] if isinstance(instruction, list) else instruction
        if training_task == 't2m':
            one_input_ids, one_target_ids = build_one_instance_t2m(tokenizer, caption, motion)
        elif training_task == 'm2t':
            one_input_ids, one_target_ids = build_one_instance_m2t(tokenizer, caption, motion, instruction=instr)
